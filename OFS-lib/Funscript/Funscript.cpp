...
void Funscript::OptimizeWavesInSelection(int32_t tolerance, OptimizeWavesPreset preset) noexcept
{
	OFS_PROFILE(__FUNCTION__);
	if (!HasSelection() || data.Selection.size() < 2) return;
	if (tolerance < 0) tolerance = 0;
	if (tolerance > 100) tolerance = 100;

	// 1. Snapshot selected actions sorted by time.
	std::vector<FunscriptAction> sel;
	sel.reserve(data.Selection.size());
	for (const auto& a : data.Selection) sel.push_back(a);
	std::sort(sel.begin(), sel.end(),
		[](const FunscriptAction& a, const FunscriptAction& b) { return a.atS < b.atS; });

	// 2. Walk and collect indices of "significant" extrema. A reversal is
	//    significant only when the swing away from the running extreme
	//    exceeds tolerance — small wobbles are absorbed into the dominant
	//    direction so a sequence like 0,60,40,100 reads as one wave.
	std::vector<int> keepIdx;
	keepIdx.push_back(0);

	int direction = 0; // 0=unknown, +1=ascending, -1=descending
	int runIdx = 0;
	int32_t runPos = sel[0].pos;

	for (int i = 1; i < (int)sel.size(); ++i) {
		int32_t p = sel[i].pos;
		if (direction == 0) {
			if (p > runPos)      { direction = 1;  runIdx = i; runPos = p; }
			else if (p < runPos) { direction = -1; runIdx = i; runPos = p; }
			continue;
		}
		if (direction > 0) {
			if (p >= runPos) { runIdx = i; runPos = p; }
			else if (runPos - p > tolerance) {
				if (runIdx != keepIdx.back()) keepIdx.push_back(runIdx);
				direction = -1;
				runIdx = i; runPos = p;
			}
		} else {
			if (p <= runPos) { runIdx = i; runPos = p; }
			else if (p - runPos > tolerance) {
				if (runIdx != keepIdx.back()) keepIdx.push_back(runIdx);
				direction = 1;
				runIdx = i; runPos = p;
			}
	...
	if (runIdx != keepIdx.back()) keepIdx.push_back(runIdx);
	const int lastIdx = (int)sel.size() - 1;
	if (keepIdx.back() != lastIdx) keepIdx.push_back(lastIdx);

	// 3. Produce output actions per preset. Output is fully replacing the
	//    selected actions (delete-then-insert), so timing+pos are explicit.
	std::vector<FunscriptAction> output;

	if (preset == OptimizeWavesPreset::None) {
		for (int idx : keepIdx) output.push_back(sel[idx]);
	}
	else if (preset == OptimizeWavesPreset::Normalize0_100) {
		for (size_t k = 0; k < keepIdx.size(); ++k) {
			FunscriptAction a = sel[keepIdx[k]];
			int16_t newPos;
			if (k == 0) {
				if (keepIdx.size() > 1) {
					int32_t next = sel[keepIdx[1]].pos;
					newPos = (a.pos < next) ? 0 : 100;
				} else newPos = a.pos;
			} else if (k == keepIdx.size() - 1) {
				int32_t prev = sel[keepIdx[k-1]].pos;
				newPos = (a.pos > prev) ? 100 : 0;
			} else {
				int32_t prev = sel[keepIdx[k-1]].pos;
				int32_t next = sel[keepIdx[k+1]].pos;
				newPos = (a.pos > prev && a.pos > next) ? 100 : 0;
			}
			a.pos = newPos;
			output.push_back(a);
		}
		...
		// Quartile pattern for each wave (pair of consecutive kept indices).
		// 2-point waves (no wobble within) are normalized to 0/100 only.
		// 3+-point waves are replaced by 4 evenly time-spaced points whose
		// pos profile depends on direction and the "2nd-from-start" sample:
		//   ascending,  2nd>=50: 0,75,55,100   ascending,  2nd<50: 0,25,45,100
		//   descending, 2nd>=50: 100,75,55,0   descending, 2nd<50: 100,25,45,0
		for (size_t w = 0; w + 1 < keepIdx.size(); ++w) {
			const int sIdx = keepIdx[w];
			const int eIdx = keepIdx[w + 1];
			const FunscriptAction& sAct = sel[sIdx];
			const FunscriptAction& eAct = sel[eIdx];
			const int inputCount = eIdx - sIdx + 1;
			const bool ascending = eAct.pos > sAct.pos;
			const int16_t outStart = ascending ? 0 : 100;
			const int16_t outEnd   = ascending ? 100 : 0;

			if (w == 0) output.push_back(FunscriptAction(sAct.atS, outStart));

			if (inputCount <= 2) {
				output.push_back(FunscriptAction(eAct.atS, outEnd));
				continue;
			}

			const int32_t secondPos = sel[sIdx + 1].pos;
			const bool secondHigh = secondPos >= 50;
			int16_t p1, p2;
			if (secondHigh) { p1 = 75; p2 = 55; }
			else            { p1 = 25; p2 = 45; }

			const float t0 = sAct.atS;
			const float t3 = eAct.atS;
			const float dt = (t3 - t0) / 3.0f;
			output.push_back(FunscriptAction(t0 + dt,       p1));
			output.push_back(FunscriptAction(t0 + 2.f * dt, p2));
			output.push_back(FunscriptAction(t3, outEnd));
		}
	}

	// 4. Replace: remove all selected actions, then insert outputs.
	FunscriptArray toRemove;
	for (const auto& a : sel) toRemove.emplace(a);
	auto it = std::remove_if(data.Actions.begin(), data.Actions.end(),
		[&toRemove, end = toRemove.end()](auto a) { return toRemove.find(a) != end; });
	data.Actions.erase(it, data.Actions.end());

	ClearSelection();
	for (const auto& a : output) {
		data.Actions.emplace(a);
		data.Selection.emplace_back_unsorted(a);
	}
	sortSelection();
	notifyActionsChanged(true);
	checkForInvalidatedActions();
